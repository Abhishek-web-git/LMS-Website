    ...
    <form className='w-[90%] md:w-200 h-150 bg-[white] shadow-xl rounded-2xl flex' onSubmit={(e)=>e.preventDefault()}>
      ...
      <div className="md:w-[50%] w-[100%] h-[100%] flex flex-col items-center justify-center gap-3">
        ...
        <div className="flex flex-col gap-1 w-[80%] items-start justify-center px-3">
          ...
          <input id="name" type="text" className='border-[1px] bg-white h-[35px] border-[#e7e6e6] text-[15px] px-[20px]' placeholder="Enter Your Name" onChange={(e)=>setName(e.target.value)} value={name}/>
        </div>
        <div className="flex flex-col gap-1 w-[80%] items-start justify-center px-3">
          <label htmlFor="email" className="font-semibold">Email</label>
          <input id="email" type="email" className='border-[1px] bg-white h-[35px] border-[#e7e6e6] text-[15px] px-[20px]' placeholder="Enter Your Email" onChange={(e)=>setEmail(e.target.value)} value={email}/>
        </div>
        <div className="flex flex-col gap-1 w-[80%] items-start justify-center px-3 relative">
          <label htmlFor="pass" className="font-semibold">Password</label>
          <input id="pass" type={show ? "text" : "password"} 
          className='border-[1px] bg-white h-[35px] border-[#e7e6e6] text-[15px] px-[20px]' placeholder="Enter Your Password" onChange={(e)=>setPassword(e.target.value)} value={password}/>

          { show ? ( 
            <IoEyeSharp onClick={() => setShow(false)} className="absolute w-[20px] h-[20px] cursor-pointer right-[5%] bottom-[10%]"/> ) : (
            <IoEyeOffOutline onClick={() => setShow(true)} className="absolute w-[20px] h-[20px] cursor-pointer right-[5%] bottom-[10%]" />)}
        </div>
        ...
        <div className="w-[80%] h-[40px] border-1 border-[black] rounded-[5px] flex items-center justify-center" onClick={googleSignUp}>
          ...
          <span className="text-[18px] text-gray-500 cursor-pointer">Google</span>

        </div>

        <div className="text-[#6f6f6f] cursor-pointer">already have an account??
      ...
      <div className="w-[50%] h-[100%] rounded-r-2xl bg-[black] md:flex items-center justify-center flex-col hidden">
        ...
        <span className="text-2xl text-white">LEARNING WEBSITE</span>
