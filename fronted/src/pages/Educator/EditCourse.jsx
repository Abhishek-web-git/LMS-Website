    ...
    <div className='max-w-5xl mx-auto p-6 mt-10 bg-white rounded-lg shadow-md'>
      ...
      <div className='flex items-center justify-center gap-[20px] md:justify-between flex-col md:flex-row mb-6 relative'>
        ...
        <FaArrowLeftLong className='top-[-20%] md:top-[20%] absolute left-[0] md:left-[2%] w-[22px] h-[22px] cursor-pointer ' onClick={()=>navigate("/courses")}/>
          ...
          <div className='space-x-2 space-y-2'>
            <button className='bg-black text-white px-4 py-2 rounded-md cursor-pointer'>Go to Lecture Page</button>
          </div>
      ...
      <div className='bg-gray-50 p-6 rounded-md'>
        ...
        <div className='space-x-2 space-y-2'>
          ...
          {!isPublished ? <button className='bg-green-100 text-green-600 px-4 py-2 rounded-md border-1 cursor-pointer' onClick={()=>setIsPublished(prev=>!prev)}>Click to Publish</button> : <button className='bg-red-100 text-red-600 px-4 py-2 rounded-md border-1' onClick={()=>setIsPublished(prev=>!prev)}>Click to UnPublish</button>}

          <button className='bg-red-600 text-white px-4 py-2 rounded-md cursor-pointer' onClick={handleRemoveCourse}>Remove Course</button>
        </div>
...
{/* form */}
        ...
        <form className='space-y-6' onSubmit={(e)=>e.preventDefault()}>
          <div>
            ...
            <input type="text" id='title' className='w-full border px-4 py-2 rounded-md' placeholder='CourseTitle' onChange={(e)=>setTitle(e.target.value)} value={title}/>
          </div>
          ...
          <div className='flex flex-col sm:flex-row sm:space-x-4 space-y-4 sm:space-y-0'>
            ...
            <div className='flex-1'>
              ...
              <select name="" id="category" className='w-full border px-4 py-2 rounded-md bg-white cursor-pointer' onChange={(e)=>setCategory(e.target.value)} value={category}>
                ...
                <option value="Block Chain Design">Block Chain Design</option>
            ...
            <div className='flex-1'>
              ...
              <select name="" id="level" className='w-full border px-4 py-2 rounded-md bg-white cursor-pointer' onChange={(e)=>setLevel(e.target.value)} value={level}>
            ...
            <div className='flex-1'>
              ...
              <input type="number" id='price' name='' className='w-full border px-4 py-2 rounded-md' placeholder='₹' onChange={(e)=>setPrice(e.target.value)} value={price}/>
            </div>

          </div>
          <div>
              <label htmlFor="" className='block font-medium text-gray-700 mb-1 text-sm'>Course Thumnails</label>
              <input type="file" hidden ref={thumb} accept='image/*' onChange={handleThumnail}/>
            </div>
            ...
            <div className='flex items-center justifty-start gap-[15px]'>
